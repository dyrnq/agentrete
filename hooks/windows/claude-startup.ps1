# agentrete Claude Code startup hook (Windows PowerShell)

$AGENTRETE_URL = if ($env:AGENTRETE_URL) { $env:AGENTRETE_URL } else { "http://127.0.0.1:9092" }
$project = (Split-Path -Leaf (Get-Location))
$cacheFile = Join-Path $env:TEMP "agentrete-claude-startup-$project.cache"

if (Test-Path $cacheFile) {
    $lastWrite = (Get-Item $cacheFile).LastWriteTime
    if ((Get-Date) - $lastWrite -lt [TimeSpan]::FromHours(1)) { exit 0 }
}
...
try {
    ...
    if ($result.result.content) {
        Write-Host "agentrete: project memories for $project" -ForegroundColor Cyan
        foreach ($c in $result.result.content) {
            Write-Host "  $($c.text)" -ForegroundColor Gray
        }
    }
} catch {}
