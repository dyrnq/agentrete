...
pub(crate) async fn initialize(pool: &SqlitePool) -> Result<()> {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_memories_created_at ON memories(created_at)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE UNIQUE INDEX IF NOT EXISTS idx_memories_content_type ON memories(content, type) WHERE deleted_at IS NULL").execute(pool).await?;
    ...
    let _ = sqlx::query("CREATE INDEX IF NOT EXISTS idx_kg_triples_subject ON kg_triples(subject)")
        .execute(pool)
        .await;
    let _ = sqlx::query("CREATE INDEX IF NOT EXISTS idx_kg_triples_object ON kg_triples(object)")
        .execute(pool)
        .await;
    let _ =
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_kg_triples_predicate ON kg_triples(predicate)")
            .execute(pool)
            .await;
    let _ = sqlx::query(
    ...
    .execute(pool)
    .await;
    let _ = sqlx::query("CREATE INDEX IF NOT EXISTS idx_kg_triples_project ON kg_triples(project)")
        .execute(pool)
        .await;
...
pub(crate) async fn init_vec(pool: &SqlitePool, vec_dims: usize) -> Result<()> {
    ...
    let needs_rebuild = vec_dims > 0 && stored_dims.is_some_and(|d| d as usize != vec_dims);
    if needs_rebuild {
        log::info!(
