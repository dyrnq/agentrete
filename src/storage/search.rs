//! Search engines: RRF fusion, vec0 KNN, FTS5 BM25, cosine rerank + row types + vector math.
...
pub(crate) async fn search_rrf(
    ...
    let qv: Option<Vec<f32>> = if vec_enabled {
    ...
    let (mut vec_results, fts_results) = if let Some(ref qv) = qv {
        ...
        (vr.unwrap_or_default(), fr?)
    } else {
        (
            vec![],
            search_fts(pool, query, limit.min(MAX_LIMIT), memory_type).await?,
        )
    };
    ...
    if vec_results.is_empty() {
        if !fts_results.is_empty() {
            ...
            for item in &mut r {
                item.score = decay(item.score, &item.created_at);
            }
            log::info!("rrf: FTS5-only ({} results)", r.len());
            return Ok(r);
        }
        if embedder.is_some() && has_qv {
