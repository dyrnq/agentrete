...
async fn async_main(cli: Cli, cfg: crate::config::Config) -> anyhow::Result<()> {
    let embedder: Option<std::sync::Arc<crate::embed::embeddings::Embedder>> =
        if cfg.embedding.backend != crate::config::EmbeddingBackend::None {
            eprintln!(
    ...
    let store = storage::Store::open(&cfg, embedder.clone()).await?;
    ...
    match cli.command {
        ...
        Commands::Mcp { port } => {
            ...
            let embed_handle = if is_http && embedder.is_some() {
