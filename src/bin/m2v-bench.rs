...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "500 texts: {:.3}ms ({:.2}ms each)",
        t.elapsed().as_secs_f64() * 1000.0,
        t.elapsed().as_secs_f64() * 2.0
    );

    let texts_1000: Vec<String> = (0..1000)
        .map(|_| "长文本测试: Rust agent memory system with sqlite-vec KNN search".into())
        .collect();
    let t = Instant::now();
    let _ = model.encode(&texts_1000);
    println!(
        "1000 texts: {:.3}ms ({:.2}ms each)",
        t.elapsed().as_secs_f64() * 1000.0,
