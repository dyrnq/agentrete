...
fn home_cache_dir() -> std::path::PathBuf {
    let base = std::env::var("HF_HOME")
        ...
        .or_else(|_| {
            std::env::var("XDG_CACHE_HOME").map(|d| std::path::PathBuf::from(d).join("huggingface"))
        })
        .unwrap_or_else(|_| {
...
impl CandleEmbedBuilder {
    ...
    pub fn new() -> Self {
        Self {
            ...
            embedding_model: WithModel::Custom("moka-ai/m3e-base".to_string()),
            mean_pooling: true,
    ...
    pub fn build(self) -> Result<BasedBertEmbedder> {
        ...
        // Try local HuggingFace cache first, fall back to hf-hub download
        let cache_dir = home_cache_dir()
            .join(format!("models--{}", model_id.replace("/", "--")))
            .join("snapshots");

        eprintln!(
            "DEBUG: model_id={}, cache_dir={}, exists={}",
            model_id,
            cache_dir.display(),
            cache_dir.exists()
        );
        ...
        let (config_filename, tokenizer_filename, weights_filename) = {
            // Check if model files exist in the cache (any snapshot directory)
            let snap = if cache_dir.exists() {
            ...
            let use_local = snap.as_ref().is_some_and(|s| {
                s.join("config.json").exists()
                    && s.join("tokenizer.json").exists()
                    && s.join("model.safetensors").exists()
            });

            if use_local {
                let snap = snap.unwrap();
                eprintln!("Loading model from local cache: {}", snap.display());
                (
