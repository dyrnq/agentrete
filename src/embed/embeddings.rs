...
/// The embedder that agentrete uses, selected at startup.
pub enum Embedder {
    /// On-device BERT model via candle.
...
impl Embedder {
    /// Create the appropriate embedder from config.
    ...
    pub fn embed_one(&self, text: &str) -> Result<Vec<f32>> {
        match self {
            ...
            Embedder::Remote(remote) => remote.embed_one(text),
        }
    }
}
